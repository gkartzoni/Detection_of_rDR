...
def endsemble(models,select,folder2save,weighted,experiment_name):
    ...
    # select 1 for kaggle, 2 for messidor
    if select == 0:
    ...
    if weighted == 1:
        i = 0
        coef = [2.5,2.5,5]
       # coef = [0.25,0.25,0.25,0.25,2,2,4.75,0.25]
        for model in models: 
            y_pred = list( map(add, y_pred, coef[i]* (model.predict(X_test)) ) )
            i = i + 1
        y_pred = [X_temp / 10 for X_temp in y_pred]
        fpr, tpr, threshold = roc_curve(Y, y_pred)
        roc_auc = auc(fpr, tpr)
    elif weighted == 0: 
        i = 0       
        for model in models:
            y_pred = list( map(add, y_pred, (model.predict(X_test)) ) )
        y_pred = [X_temp / l for X_temp in y_pred]
        fpr, tpr, threshold = roc_curve(Y, y_pred)
        roc_auc = auc(fpr, tpr)
...
#######################################################    
folder2save = 'Kaggle_EF_notWeighted'
...
src = '/home/gkartzoni/thesis/source/ensembleEarlyFusion.py'
dst = evaluation_folder + '/ensembleEarlyFusion.py'
copyfile(src, dst)	

with open(evaluation_folder+'/metrics.json', 'w') as json_file:  
     json.dump(json_metrics, json_file)
#######################################################################
folder2save = 'Messidor_EF_notWeighted'
...
##############################################
# folder2save = 'Kaggle_EF_Weighted'
...
# #################################################
# folder2save = 'Messidor_EF_Weighted'
